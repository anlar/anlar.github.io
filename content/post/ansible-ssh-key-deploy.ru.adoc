...
При развёртывании систем бывает нужно сгенерировать SSH-ключи, которые будут использовать удалённые хосты для доступа друг к другу. Здесь приведён пример плейбука для Ansible, который добавляет на хост `head` новый SSH-ключ и деплоит его на хосты `node*`. Т.е. хост `head` будет ходить по SSH на хосты `node*`.
